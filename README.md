...
- **Jobs** (Mac must be awake): **3:30 PM Chicago** weekdays refresh
  ...
  `data/buy_signals/` and emails the top 20 if
  `~/.config/api/notify-email.txt` (or `STOCK_PICKER_NOTIFY_EMAIL`) is set.
  The Trading tab loads that file on click.
...
| Weekdays 8:32 AM Chicago | Score today’s dated opens → `data/buy_signals/{date}.json` and `latest.json`; email top 20 |
